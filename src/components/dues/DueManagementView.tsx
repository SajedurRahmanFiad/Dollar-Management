...
export const DueManagementView: React.FC<DueManagementViewProps> = ({
                ...
                >
                  <div>
                    ...
                    <div className="text-xs font-extrabold text-slate-800 mt-1">
                      {formatUsd(deal.dollarAmount)} USD @ ৳{deal.exchangeRate}
                    </div>
  ...
  const handleCopyReminder = (debtor: (typeof debtors)[0]) => {
    ...
    }, friendly reminder regarding outstanding balance of ${formatBdt(
      debtor.summary.currentDue
    )} for your recent dollar exchange (${debtor.unpaidDeals
      .map((d) => d.dealNumber)
      .join(', ')}). Please submit the payment slip at your convenience. Thank you!`;
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-10 animate-in fade-in duration-150">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-black text-slate-900 tracking-tight">
            Receivables &amp; Due Ledger
          </h1>
          <p className="text-xs text-slate-500 mt-0.5">
            Track unpaid balances across debtors and dispatch instant payment reminders
          </p>
        </div>
              ...
              >
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <button
                    ...
                    className="px-3 py-1.5 bg-slate-50 hover:bg-slate-100 border border-slate-200 text-slate-700 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5"
                  >
                    {copiedId === debtor.customer.id ? (
                      <>
